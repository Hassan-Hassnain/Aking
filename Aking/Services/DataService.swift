...
extension GDataService {
    ...
    //                        let members = task[KTask.MEMBERS]
                            ...
                            let newTask = Task(id: id, title: title, assigneeName: assigneName, projectName: projectName, dueDate: dueDate, description: description, members: [], tag: tag, color: taskColor ?? UIColor.white, status: sts)
    ...
    func convertTaskToDictionary(task: Task) -> Dictionary<String, Any> {
        ...
        let taskDict: Dictionary<String, Any> =  [
            ...
            KTask.MEMBERS : task.members,//convertArrayToDictionary(array: task.members),
            KTask.TAG : task.tag,
            KTask.COLOR : task.color.toRGBAString(),
            KTask.STATUS : task.status == .done ? true : false
        ]
        
        print(convertArrayToDictionary(array: task.members))
        return taskDict
    }
    
    func convertArrayToDictionary (array: [String]) -> Dictionary<Int, String> {
        var dict: [Int: String] = [:]
